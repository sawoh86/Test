...
- **🎯 벡터 전계 지원**: Ex, Ey, Ez 성분을 모두 고려한 완전한 벡터 전자기장 변환
- **🔄 편파 분석**: 구면 좌표계 변환 (E_theta, E_phi) 및 편파 비율 분석
- **⚡ Poynting 벡터**: 자기장 및 에너지 흐름 계산
- **📊 다양한 안테나 모델**: 쌍극자(X/Y/Z), 패치, 슬롯 안테나

## 기능

### 버전 2.0 (벡터 전계 지원)
- **벡터 전계 변환**: Ex, Ey, Ez 성분 모두 처리
- **구면 좌표 변환**: E_theta, E_phi 편파 성분 계산
- **자기장 계산**: Hx, Hy, Hz 성분 자동 계산
- **Poynting 벡터**: 에너지 흐름 및 방사 패턴 분석
- **편파 분석**: E_theta/E_phi 비율 및 교차편파 분석
- CSV 형식: `Ex_mag`, `Ex_phase`, `Ey_mag`, `Ey_phase`, `Ez_mag`, `Ez_phase`
